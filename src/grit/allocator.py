...
class DateAllocator:
    ...
    def get_next_date(self, current_counts: dict[str, int] = None) -> str:
        """
        Determines the next date to allocate a commit to based on the configured
        strategy.
        ...
        with self.state._conn as conn:
            ...
            for date_str, db_count in rows:
                ...
                count = current_counts.get(
                    date_str, db_count
                    ) if current_counts else db_count
    ...
    def get_status_allocations(self) -> list[dict]:
        ...
        2. Next Available: The first date where a new commit will land (following the
        streak logic).
        ...
        # Use the same priority logic as get_next_date to find the next few available
        # slots.
        query = f"""
        WITH RECURSIVE dates(d) AS (
            SELECT ? 
